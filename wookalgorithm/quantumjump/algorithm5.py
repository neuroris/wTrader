...
'''
Quantum Jump Algorithm (2021, 04, 02)

Discrete price algorithm using inverse only
1. Interval play but sell below reference price (start on bid price)
...
class QAlgorithm5(AlgorithmBase):
    ...
    def update_transaction_info(self, item):
        ...
        if item.current_price > self.reference_price:
            ...
            self.inverse.correct_sales(self.reference_price - self.loss_cut)
        ...
        if order.order_position in (PURCHASE, CORRECT_PURCHASE):
            ...
            if order.executed_amount:
                ...
                if order_amount >= self.minimum_transaction_amount:
                    ...
                    self.inverse.sell(self.reference_price - self.loss_cut, order_amount)
