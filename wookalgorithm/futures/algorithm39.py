...
from wookitem import Item, Order, Episode, AlgorithmItem, FuturesAlgorithmItem, Timeline, OrderStatus
...
'''
Original Algorithm (2021, 03, 14)
** New chart System based on real KOSPI200!!
1. Price average first degree regression model
2. Buy and Hold strategy 
3. New chart system based on volume
4. Multiple episode
...
10. Settle at constant interval automatically
11. Divided minute chart system
12. Limit order
13. Moving Average Slope!!!
'''

class FMAlgorithm39(FuturesAlgorithmBase):
    def __init__(self, trader, log):
        ...
        self.quantum_volume = 100
        self.contract_time = 0

        self.strategy = TIME_INTERVAL_STRATEGY
        self.previous_time = 0
        self.previous_accumulated_profit = 0
        self.accumulated_profit = 0
        self.previous_bet_count = 0
        self.bet_count = 0

        # Betting variables
        self.bet_amount = 2
        self.predict_ahead = 3
        self.candlestick_duration = 60
        self.point_duration = 8
        self.inverse_criteria = 6
        self.mean_count = 4
        self.maximum_consecutive_slope_counts = 8
        ...
        self.default_r1_interval = 3
        self.r1_interval = 4
        self.r3_interval = 9
        self.par1_slope_interval = 10
    ...
    def start(self, broker, capital, interval, loss_cut, fee, minimum_transaction_amount):
        ...
        # Charting & Monitoring
        self.futures = FuturesAlgorithmItem('101S6000')
        self.kospi200 = FuturesAlgorithmItem(KOSPI200_CODE)
        self.add_item(self.futures)
        self.add_item(self.kospi200)
        self.initialize(broker, capital, interval, loss_cut, fee, minimum_transaction_amount)
        self.clear_open_orders()
        # self.chart_item = self.kospi200
        self.chart_item = self.futures
        # self.criteria_item = self.kospi200
        self.criteria_item = self.futures
        self.trading_item = self.futures

        broker.chart_prices.clear()
        broker.request_futures_stock_price_min(self.futures.item_code)
        broker.event_loop.exec()
        broker.request_kospi200_index()
        broker.event_loop.exec()
        broker.demand_monitoring_items_info(self.futures)
        broker.demand_monitoring_items_info(self.kospi200)
        self.timer.start()
    ...
    def market_status(self, item):
        ...
        # chart = self.trading_item.chart
        chart = self.criteria_item.chart
        if item.item_code == self.kospi200.item_code:
            # interval = chart.index[(self.r1_interval * -2):]
            interval = chart.index[-40:]
            # self.debug(chart.loc[interval, ['PAR1_Slope', 'PAR1_SlopeSlope', 'PAR1_CorrectedSlope', 'point', 'position']])
            self.debug(chart.loc[interval, ['PAR1_CorrectedSlope', 'point', 'position_value', 'position', 'MASlope']])
            # self.debug('accumulated profit({})'.format(self.bet_count), self.accumulated_profit)
            self.debug(chart.loc[interval, ['position', 'MA5R1_Slope']])

            # max_point = chart.point[-self.point_duration:].max()
            # min_point = chart.point[-self.point_duration:].min()
            # inverse_point = max(max_point, abs(min_point))
            # self.debug(max_point, min_point, inverse_point)

        if item.item_code == self.criteria_item.item_code:
    ...
    def initiate_order(self, amount=0, price=0, order_type='LIMIT'):
        if len(self.chart_item.chart) < 4:
            self.post_magenta('Chart length is less than 4')
            return

        # self.trade()
        # self.previous_bet_count = 1
        # self.bet_count = 1
    ...
    def trade(self):
        ...
        elif self.strategy == TIME_INTERVAL_STRATEGY:
            # episode_amount = self.episode_amount
            episode_amount = 1
            corrected_slope = self.kospi200.chart.PAR1_CorrectedSlope[-1]
            purchase_target_amount = episode_amount if corrected_slope >= 0 else -episode_amount
            sale_target_amount = episode_amount if corrected_slope < 0 else -episode_amount
    ...
    def monitor_trend_deviation(self, current_price):
        if self.strategy != TIME_INTERVAL_STRATEGY:
            return

        if len(self.chart_item.chart) < self.r1_interval:
            return

        if self.previous_bet_count == self.bet_count:
            return
        else:
            self.previous_bet_count = self.bet_count

        criteria_chart = self.criteria_item.chart
        # chart = self.trading_item.chart
        chart = self.criteria_item.chart

        current_slope = chart.MA5R1_Slope[-1]
        current_time = chart.index[-1]
        previous_position = chart.position[-self.predict_ahead - 1]
        consecutive_slope_count = self.get_consecutive_slope_count(chart)

        # Previous Position
        for index in range(self.predict_ahead - 1):
            if chart.position[-1 - index - 1] not in PURCHASE_SELL:
                previous_position = OUTSET_POSITION
                break

        # Anticipated position
        anticipated_position = LONG_POSITION
        if current_slope >= 0:
            if consecutive_slope_count > self.maximum_consecutive_slope_counts:
                anticipated_position = SHORT_POSITION
        else:
            if consecutive_slope_count <= self.maximum_consecutive_slope_counts:
                anticipated_position = SHORT_POSITION

        if chart.MA5R1_Slope[-1] * chart.MA5R1_Slope[-2] < 0:
            previous_consecutive_slope_count = self.get_consecutive_slope_count(chart, 1)
            if previous_consecutive_slope_count > self.maximum_consecutive_slope_counts:
                anticipated_position = SETTLE_POSITION

        # if chart.position[-2] == SETTLE_POSITION or chart.position[-3] == SETTLE_POSITION:
        if chart.position[-2] == SETTLE_POSITION:
            anticipated_position = HOLD_POSITION

        self.trader.le_test.setText(str(previous_position))
        # self.trader.le_order_number.setText(anticipated_position)
        self.trader.le_order_number.setText(str(chart.position[-2]))

        # Trade by anticipated position
        if anticipated_position == LONG_POSITION:
            chart.loc[current_time, 'position'] = SELL
            if previous_position == PURCHASE:
                self.sell(self.bet_amount * 2, 0, 'MARKET')
            elif previous_position not in PURCHASE_SELL:
                self.sell(self.bet_amount, 0, 'MARKET')
        elif anticipated_position == SHORT_POSITION:
            chart.loc[current_time, 'position'] = PURCHASE
            if previous_position == SELL:
                self.buy(self.bet_amount * 2, 0, 'MARKET')
            elif previous_position not in PURCHASE_SELL:
                self.buy(self.bet_amount, 0, 'MARKET')
        elif anticipated_position == SETTLE_POSITION:
            chart.loc[current_time, 'position'] = SETTLE_POSITION
            if self.futures.holding_amount > 0:
                self.sell(self.futures.holding_amount, 0, 'MARKET')
            elif self.futures.holding_amount < 0:
                self.buy(-self.futures.holding_amount, 0, 'MARKET')
        elif anticipated_position == HOLD_POSITION:
    ...
    def buy(self, amount=0, given_price=0, order_type='LIMIT'):
        ...
        if self.futures.holding_amount >= 0:
            price = self.get_price_average(self.futures)
        else:
            price = self.get_buy_limit(self.futures)
        ...
        self.futures.buy(price, amount, 'MARKET')
    ...
    def update_long_episode(self, order):
        ...
        if executed_amount:
            ...
            # Contract time
            chart_len = len(self.futures.chart)
            self.contract_time = chart_len

            # History
            x = chart_len - 1
            self.long_episode_history[self.long_episode.episode_number] = (x, order.executed_price_avg)
    ...
    def update_short_episode(self, order):
        ...
        if executed_amount:
            ...
            # History
            x = chart_len - 1
            self.short_episode_history[self.short_episode.episode_number] = (x, order.executed_price_avg)
    ...
    def customize_past_chart(self, item):
        ...
        chart['position'] = numpy.nan
        chart['position_value'] = 0
        chart['MA5R1'] = 0
        # chart[['PAR1', 'PAR1_Slope', 'PAR1_SlopeSlope', 'BL', 'SL', 'LLL', 'ULL']] = 0
        # chart[['PAR3', 'PAR3_Diff', 'PAR3_DiffDiff', 'PAR3_DiffDiffDiff']] = 0
        # chart[['MA5R3', 'MA5R3_Diff', 'MA5R3_DiffDiff', 'MA5R3_DiffDiffDiff']] = 0

        x1_len = self.r1_interval if chart_len > self.r1_interval else chart_len
        x3_len = self.r3_interval if chart_len > self.r3_interval else chart_len

        x1, PAR1 = self.get_linear_regression(chart, chart.PA, x1_len)
        x3, PAR3 = self.get_cubic_regression(chart, chart.PA, x3_len)
        x3, MA5R3 = self.get_cubic_regression(chart, chart.MA5, x3_len)

        # PAR1 Slope
        slope_count = chart_len - 1
        # slope_count = self.r1_interval * 2 if slope_count > self.r1_interval * 2 else slope_count
        for offset in range(slope_count):
            interval = chart_len - offset
            interval = self.r1_interval if interval > self.r1_interval else interval
            x_PA, R1_PA = self.get_linear_regression(chart, chart.PA, interval, offset)
            x_MA5, R1_MA5 = self.get_linear_regression(chart, chart.MA5, interval, offset)
            slope_PA = numpy.polyfit(x_PA, R1_PA, 1)[0]
            slope_MA5 = numpy.polyfit(x_MA5, R1_MA5, 1)[0]
            index = chart.index[-1 - offset]
            chart.loc[index, 'PAR1_Slope'] = slope_PA
            chart.loc[index, 'MA5R1_Slope'] = slope_MA5

        # PAR1 Slope Slope & PAR1 Corrected slope
        slopeslope_count = slope_count - 1
        # slopeslope_count = self.r1_interval if slopeslope_count > self.r1_interval else slopeslope_count
        for offset in range(slopeslope_count):
            interval = slope_count - offset
            interval = self.r1_interval if interval > self.r1_interval else interval
            x, R1 = self.get_linear_regression(chart, chart.PAR1_Slope, interval, offset)
            slopeslope = numpy.polyfit(x, R1, 1)[0]
            index = chart.index[-1 - offset]
            chart.loc[index, 'PAR1_SlopeSlope'] = slopeslope

            slope = chart.loc[index, 'PAR1_Slope']
            corrected_slope = slope + slopeslope
            chart.loc[index, 'PAR1_CorrectedSlope'] = corrected_slope
            chart.loc[index, 'position_value'] = 1 if corrected_slope >= 0 else -1

        # Point
        chart['point'] = chart.position_value.rolling(self.point_duration, 1).sum()

        # MeanSlope
        chart['MASlope'] = chart.PAR1_CorrectedSlope.rolling(self.mean_count, 1).mean()
        ...
        chart.loc[x3_interval, 'MA5R3'] = MA5R3
        chart.loc[x3_interval, 'MA5R3_Diff'] = chart.MA5R3.diff().fillna(0)
        chart.loc[x3_interval, 'MA5R3_DiffDiff'] = chart.MA5R3_Diff.diff().fillna(0)
        chart.loc[x3_interval, 'MA5R3_DiffDiffDiff'] = chart.MA5R3_DiffDiff.diff().fillna(0)

    def update_custom_chart(self, item):
        chart = item.chart
        chart_len = len(chart)

        ma5 = -5 if chart_len > 5 else -chart_len
        ma10 = -10 if chart_len > 10 else -chart_len
        ma20 = -20 if chart_len > 20 else -chart_len
        diff = -2 if chart_len > 1 else -1
        ...
        chart.loc[current_time, 'Diff5'] = round(chart.MA5[-1] - chart.MA5[diff], 3)
        chart.loc[current_time, 'Diff10'] = round(chart.MA10[-1] - chart.MA10[diff], 3)
        chart.loc[current_time, 'Diff20'] = round(chart.MA20[-1] - chart.MA20[diff], 3)
        chart.loc[current_time, 'DiffDiff5'] = round(chart.Diff5[-1] - chart.Diff5[diff], 3)
        chart.loc[current_time, 'DiffDiff10'] = round(chart.Diff10[-1] - chart.Diff10[diff], 3)
        chart.loc[current_time, 'DiffDiff20'] = round(chart.Diff20[-1] - chart.Diff20[diff], 3)
        ...
        if chart_len > 2:
            ...
            chart.loc[current_time, 'PAR1_Slope'] = PAR1_Slope

        # PAR1 Slope Slope & Corrected Slope
        if chart_len > 3:
            slope_count = chart_len - 1
            slope_interval = self.r1_interval if slope_count > self.r1_interval else slope_count
            x, R1 = self.get_linear_regression(chart, chart.PAR1_Slope, slope_interval)
            PAR1_SlopeSlope = numpy.polyfit(x, R1, 1)[0]
            chart.loc[current_time, 'PAR1_SlopeSlope'] = PAR1_SlopeSlope
            corrected_slope = PAR1_Slope + PAR1_SlopeSlope
            chart.loc[current_time, 'PAR1_CorrectedSlope'] = corrected_slope
            chart.loc[current_time, 'position_value'] = 1 if corrected_slope >= 0 else -1
            chart.loc[current_time, 'point'] = chart.position_value[-self.point_duration:].sum()
            chart.loc[current_time, 'MASlope'] = chart.PAR1_CorrectedSlope[-self.mean_count:].mean()

        # MA5 Slope
        x1, MA5R1 = self.get_linear_regression(chart, chart.MA5, x1_len)
        if chart_len > 5:
            MA5_Slope = numpy.polyfit(x1, MA5R1, 1)[0]
            chart.loc[current_time, 'MA5R1_Slope'] = MA5_Slope
        chart.loc[x1_interval, 'MA5R1'] = MA5R1

        reference_time = self.r1_interval
        futures = self.futures.chart
        previous_price = futures.Close[-reference_time]
        current_price = futures.Close[-1]
        position = 1 if chart.PAR1_CorrectedSlope[-reference_time] > 0 else -1
        profit = int((current_price - previous_price) * position * MULTIPLIER)
        chart.loc[current_time, 'Profit'] = profit
        if self.previous_time == current_time:
            self.accumulated_profit = self.previous_accumulated_profit + profit
        else:
            self.previous_accumulated_profit = self.accumulated_profit
            self.accumulated_profit += profit
            if item.item_code == self.trading_item.item_code:
        ...
        chart.loc[x3_interval, 'PAR3_Diff'] = chart.PAR3.diff().fillna(0)
        chart.loc[x3_interval, 'PAR3_DiffDiff'] = chart.PAR3_Diff.diff().fillna(0)
        chart.loc[current_time, 'PAR3_DiffDiffDiff'] = chart.PAR3_DiffDiff[-1] - chart.PAR3_DiffDiff[diff]
        chart.loc[x3_interval, 'MA5R3'] = MA5R3
        chart.loc[x3_interval, 'MA5R3_Diff'] = chart.MA5R3.diff().fillna(0)
        chart.loc[x3_interval, 'MA5R3_DiffDiff'] = chart.MA5R3_Diff.diff().fillna(0)
        chart.loc[current_time, 'MA5R3_DiffDiffDiff'] = chart.MA5R3_DiffDiff[-1] - chart.MA5R3_DiffDiff[diff]
    ...
    def display_chart(self):
        ...
        chart = self.chart_item.chart

        chart_len = len(chart)
        if not chart_len: return
        ...
        min_price = self.get_min_price(chart, x1, x2)
        max_price = self.get_max_price(chart, x1, x2)
        y1 = min_price - 0.1
        y2 = max_price + 0.1
        ax.set_xlim(x1, x2)
        ax.set_ylim(y1, y2)
        ...
        # Regression
        x1_len = chart_len if chart_len < self.r1_interval else self.r1_interval
        x3_len = chart_len if chart_len < self.r3_interval else self.r3_interval
        XR1 = numpy.arange(chart_len - x1_len, chart_len)
        ax.plot(XR1, chart.PAR1[-x1_len:], color='DarkOrange')
        ax.plot(XR1, chart.ULL[-x1_len:], color='Gray')
        ax.plot(XR1, chart.LLL[-x1_len:], color='Gray')
        ax.plot(XR1, chart.BL[-x1_len:], color='DarkGray')
        ax.plot(XR1, chart.SL[-x1_len:], color='DarkGray')
        XR3 = numpy.arange(chart_len - x3_len, chart_len)
        ax.plot(XR3, chart.PAR3[-x3_len:], color='Cyan')
        ax.plot(XR3, chart.MA5R3[-x3_len:], color='DarkSlateGray')

        # ax.plot(XR1, chart.MA5R1[-x1_len:], color='red')

        # After price data acquired
        if self.start_time:
            self.annotate_chart(chart, current_time, x1, x2, y1, y2)
    ...
    def annotate_chart(self, chart, current_time, x1, x2, y1, y2):
        # Start time
        ...
        try:
            ...
            for episode_number, (x, price) in long_episode_history.items():
                if x > x1:
                    previous_episode_number = Episode.previous_number(episode_number)
                    previous_price = 0
                    if previous_episode_number in long_episode_history:
                        previous_price = long_episode_history[previous_episode_number][1]
                    # y = price - 0.03 if price != previous_price else price - 0.06
                    y = price - 0.03 if price != previous_price else price - 0.09
                    self.trader.chart_ax.plot(x, price, marker='o', markersize=4, color='GreenYellow')
                    self.trader.chart_ax.text(x - 0.3, y, episode_number)
            short_episode_history = copy.deepcopy(self.short_episode_history)
            for episode_number, (x, price) in short_episode_history.items():
                if x > x1:
                    previous_episode_number = Episode.previous_number(episode_number)
                    previous_price = 0
                    if previous_price in short_episode_history:
                        previous_price = short_episode_history[previous_episode_number][1]
                    # y = price + 0.01 if price != previous_price else price + 0.04
                    y = price + 0.01 if price != previous_price else price + 0.08
    ...
    def get_regression(self, chart, target, interval, offset, predict_len, polynomial_features):
        chart_len = len(chart)
        interval = self.r3_interval if not interval else interval
        interval = chart_len if chart_len < interval else interval
        ...
        self.linear_regression.fit(x_fitted, target.values[x1:x2])
        y_regression = self.linear_regression.predict(x_fitted)

        return x_regression, y_regression

    def get_linear_regression(self, chart, target, interval=0, offset=0, predict_len=0):
        x_regression, y_regression = \
            self.get_regression(chart, target, interval, offset, predict_len, self.polynomial_features1)
        return x_regression, y_regression

    def get_quadratic_regression(self, chart, target, interval=0, offset=0, predict_len=0):
        x_regression, y_regression = \
            self.get_regression(chart, target, interval, offset, predict_len, self.polynomial_features2)
        return x_regression, y_regression

    def get_cubic_regression(self, chart, target, interval=None, offset=0, predict_len=0):
        x_regression, y_regression = \
            self.get_regression(chart, target, interval, offset, predict_len, self.polynomial_features3)
        return x_regression, y_regression

    def get_price_average(self, item):
        price_average = float(item.chart.PAR1[-1])
        if item.chart.PAR1_Slope[-1] >= 0:
    ...
    def get_buy_limit(self, item):
        buy_limit = float(item.chart.PAR1[-1]) - self.interval
        corrected_buy_limit = wmath.get_nearest_bottom(buy_limit)
        return corrected_buy_limit

    def get_sell_limit(self, item):
        sell_limit = float(item.chart.PAR1[-1]) + self.interval
        corrected_sell_limit = wmath.get_nearest_top(sell_limit)
        return corrected_sell_limit

    def get_max_price(self, chart, x1, x2):
        max_price = chart.High[x1:x2].max()
        return max_price

    def get_min_price(self, chart, x1, x2):
        min_price = chart.Low[x1:x2].min()
        return min_price

    def get_consecutive_slope_count(self, chart, offset=0):
        current_slope = chart.MA5R1_Slope[-1 - offset]
        count = 1
        while True:
            previous_slope = chart.MA5R1_Slope[-1 - offset - count]
            criteria = previous_slope * current_slope
            if criteria >= 0:
                count += 1
            else:
                break
