...
'''
Original Algorithm (2021, 05, 04)
1. Price average first degree regression model
2. Multiple episode
3. Moving Average Slope MA5
4. Trade at turning points
5. Settle when long consecutive trend
'''

class FMAlgorithm43(FuturesAlgorithmBase):
    def __init__(self, trader, log):
        ...
        self.mean_count = 4
        self.maximum_consecutive_slope_counts = 12
    ...
    def market_status(self, item):
        ...
        # if item.item_code == self.futures.item_code:
        #     self.futures.update_profit(item.current_price)
        #     self.debug(item.current_price, self.futures.holding_amount, self.futures.profit)
    ...
    def monitor_trend_deviation(self, current_price):
        ...
        # Settle condition
        # if consecutive_slope_count > 3:
        #     if self.futures.profit < -400000:
        #         anticipated_position = SETTLE_POSITION
        if consecutive_slope_count > self.maximum_consecutive_slope_counts:
            anticipated_position = SETTLE_POSITION

        # Hold position
        if chart.position[-2] in (SETTLE_POSITION, HOLD_POSITION):
        ...
        if anticipated_position == LONG_POSITION:
            ...
            if not self.futures.holding_amount:
                ...
                self.sell(self.episode_amount * 2, 0, 'MARKET')
        elif anticipated_position == SHORT_POSITION:
            chart.loc[current_time, 'position'] = PURCHASE
            if not self.futures.holding_amount:
                self.buy(self.episode_amount, 0, 'MARKET')
            elif self.futures.holding_amount < 0:
                self.buy(self.episode_amount * 2, 0, 'MARKET')
