...
10. Long / Short episode system
'''

class FMAlgorithm9(FuturesAlgorithmBase):
    def __init__(self, trader, log):
        ...
        self.last_executed_episode = None
    ...
    def strangle_long(self):
        target_open_amount = self.strangle_episode_amount - self.futures.virtual_holding_amount
        if target_open_amount > 0:
            current_open_amount = self.futures.get_open_purchases()
            order_amount = target_open_amount - current_open_amount
            if order_amount:
                self.buy(order_amount)

    def strangle_short(self):
        target_open_amount = self.strangle_episode_amount + self.futures.virtual_holding_amount
        if target_open_amount > 0:
            current_open_amount = self.futures.get_open_sales()
            order_amount = target_open_amount - current_open_amount
            if order_amount:
                self.sell(order_amount)
    ...
    def update_long_episode(self, order):
        self.post_blue('(LONG1)', self.long_episode.episode_number, 'order', self.long_episode.order_amount,
                       'executed', self.long_episode.executed_amount_sum,
                       'open', self.long_episode.open_amount,
        ...
        # New episode creation after trade trend switch
        # elif not self.futures.virtual_holding_amount and \
        #         self.long_episode.order_amount == self.long_episode.executed_amount_sum:
        if not self.futures.virtual_holding_amount and self.long_episode.executed_amount_sum:
            self.long_episode = self.long_episode.next_episode('L')
            self.episodes[self.long_episode.episode_number] = self.long_episode

        # Cancellation of order correction right after execution
        # Cancellation of order correction right after execution
        # if self.long_episode.original_order_number == order.order_number:
        #     if self.long_episode.order_position == CORRECT_PURCHASE and order.order_state == ORDER_EXECUTED:
        #         self.post_magenta('@ ============== Got ya!! bad Kiwoom (Cancel) ==============')
        #         self.futures.cancel_purchase(order.order_number)

        if order.order_state == RECEIPT and order.order_position == PURCHASE:
            self.long_episode.order_amount += order.order_amount
            self.long_episode.open_amount += order.open_amount
            self.long_episode.virtual_open_amount += order.open_amount
        ...
        if executed_amount:
            ...
            if self.futures.holding_amount > 0 and \
                ...
                corrected_amount = previous_long_episode.executed_amount_sum - self.short_episode.executed_amount_sum
                self.long_episode.order_amount += corrected_amount
                self.long_episode.executed_amount_sum += corrected_amount
                previous_long_episode.order_amount -= corrected_amount
                previous_long_episode.executed_amount_sum -= corrected_amount
                corrected_purchase_sum = corrected_amount * previous_long_episode.purchase_price_avg
            ...
            if self.strangle_strategy:
                ...
                self.post('(COUNTER TRADE) long episode ----- SELL')
                self.sell(executed_amount)
            else:
                if self.long_episode.executed_amount_sum <= self.episode_amount:
                    self.post('(TREND TRADE) long episode ----- Trend Trade!!!!')
                    self.trade_by_trend(executed_amount)
                else:
                    self.post_magenta('@ ================ order amount exceeds episode amount ================')
                    self.post_magenta('@ ====================  no counter trade executed =====================')

            # History
            self.long_episode_history[order.executed_time] = \
                (order.executed_price_avg, self.long_episode.episode_number)

        self.post_blue('(LONG2)', self.long_episode.episode_number, 'order', self.long_episode.order_amount,
                       'executed', self.long_episode.executed_amount_sum,
                       'open', self.long_episode.open_amount,
                       'holding', self.futures.holding_amount,
                       'virtual', self.futures.virtual_holding_amount)
    ...
    def update_short_episode(self, order):
        self.post_blue('(SHORT1)', self.short_episode.episode_number, 'order', self.short_episode.order_amount,
                       'executed', self.short_episode.executed_amount_sum,
                       'open', self.short_episode.open_amount,
                       'holding', self.futures.holding_amount,
                       'virtual', self.futures.virtual_holding_amount)
        ...
        # New episode creation by trade trend switch
        # elif not self.futures.virtual_holding_amount and \
        #         self.short_episode.order_amount == self.short_episode.executed_amount_sum:
        if not self.futures.virtual_holding_amount and self.short_episode.executed_amount_sum:
            self.short_episode = self.short_episode.next_episode('S')
            self.episodes[self.short_episode.episode_number] = self.short_episode

        # Cancellation of order correction right after execution
        # Cancellation of order correction right after execution
        # if self.short_episode.original_order_number == order.order_number:
        #     if self.short_episode.order_position == CORRECT_SELL and order.order_state == ORDER_EXECUTED:
        #         self.post_magenta('@ ============== Got ya!! bad Kiwoom (Cancel) ==============')
        #         self.futures.cancel_sale(order.order_number)

        if order.order_state == RECEIPT and order.order_position == SELL:
            self.short_episode.order_amount += order.order_amount
            self.short_episode.open_amount += order.open_amount
            self.short_episode.virtual_open_amount += order.open_amount
        ...
        if executed_amount:
            ...
            self.short_episode.executed_sum += executed_amount * order.executed_price_avg
            ...
            if self.futures.holding_amount < 0 and \
                ...
                corrected_amount = previous_short_episode.executed_amount_sum - self.long_episode.executed_amount_sum
                self.short_episode.order_amount += corrected_amount
                self.short_episode.executed_amount_sum += corrected_amount
                previous_short_episode.order_amount -= corrected_amount
                previous_short_episode.executed_amount_sum -= corrected_amount

            amount_sum = self.short_episode.executed_amount_sum
            self.short_episode.executed_price_avg = round(self.short_episode.executed_sum / amount_sum, 2)
            self.futures.virtual_holding_amount += order.executed_amount
            self.short_episode.profit += order.current_profit
            self.short_episode.net_profit += order.current_net_profit

            # Episode completion
            if self.last_executed_episode == self.long_episode:
                self.long_episode = self.long_episode.next_episode('L')
                self.episodes[self.long_episode.episode_number] = self.long_episode
            self.last_executed_episode = self.short_episode
            ...
            if self.strangle_strategy:
                ...
                self.post('(COUNTER TRADE) short episode ----- BUY')
                self.buy(executed_amount)
            else:
                if self.short_episode.executed_amount_sum <= self.episode_amount:
                    self.post('(TREND TRADE) short episode ----- Trend Trade!!!!')
                    self.trade_by_trend(executed_amount)
                else:
                    self.post_magenta('@ ================ order amount exceeds episode amount ================')
                    self.post_magenta('@ ====================  no counter trade executed =====================')
        ...
        self.post_blue('(SHORT2)', self.short_episode.episode_number, 'order', self.short_episode.order_amount,
                       'executed', self.short_episode.executed_amount_sum,
                       'open', self.short_episode.open_amount,
                       'holding', self.futures.holding_amount,
                       'virtual', self.futures.virtual_holding_amount)
    ...
    def count_open_orders(self, order):
        if order.order_position == PURCHASE and order.order_state == RECEIPT:
            ...
            if not self.open_purchase_correct_orders:
                ...
                open_amount = self.futures.get_open_amount()
                cancel_amount = open_amount - self.episode_amount
                self.post_white('total open amount', open_amount)
                self.post_white('cancel amount', cancel_amount)
                # if cancel_amount > 0:
                #     self.post_magenta('@@@@@@@@@@@@@@@@@@@@@@ Wrong open amount @@@@@@@@@@@@@@@@@@@@@@')
                #     self.post_magenta('@@@@@@@@@@@@@@@@@@@@ Cancel smallest purchase order @@@@@@@@@@@')
                #     self.open_purchase_cancel_orders += 1
                #     self.futures.cancel_smallest_open_purchase(cancel_amount)
            ...
            if not self.open_sale_correct_orders:
                ...
                # if cancel_amount > 0:
                #     self.post_magenta('@@@@@@@@@@@@@@@@@@@@@@ Wrong open amount @@@@@@@@@@@@@@@@@@@@@@')
                #     self.post_magenta('@@@@@@@@@@@@@@@@@@ Cancel smallest sale order @@@@@@@@@@@@@@@@@')
                #     self.open_sale_cancel_orders += 1
                #     self.futures.cancel_smallest_open_sale(cancel_amount)
    ...
    def annotate_chart(self, current_time, x1, x2, y1, y2):
        ...
        try:
            ...
            #         self.trader.ax.plot(x, price, marker='o', markersize=4, color='GreenYellow')
