...
class FMAlgorithm44(FuturesAlgorithmBase):
    ...
    def monitor_trend_deviation(self, current_price):
        ...
        # Current slope
        current_slope = chart.PAR1_Slope[-1]

        # Previous position
        # previous_position = chart.position[-self.predict_ahead - 2]

        # Anticipated position
        # anticipated_position = LONG_POSITION if chart.MA5R1_Slope[-2] < 0 else SHORT_POSITION
        ...
        if previous_position in (NOT_AVAILABLE_POSITION, WAIT_POSITION, SETTLE_POSITION):
            if chart.MA5R1_Slope[-2] * chart.MA5R1_Slope[-3] < 0:
                if chart.MA5R1_Slope[-2] < 0:
                    chart.loc[current_time, 'position'] = PURCHASE_POSITION
                    self.buy(self.episode_amount, 0, MARKET)
                else:
                    chart.loc[current_time, 'position'] = SELL_POSITION
                    self.sell(self.episode_amount, 0, MARKET)
            else:
                chart.loc[current_time, 'position'] = WAIT_POSITION
        elif previous_position == HOLD_POSITION:
            if chart.PAR1_Slope[-1] * self.futures.holding_amount < 0:
        ...
        elif previous_position in (PURCHASE, SELL):
            chart.loc[current_time, 'position'] = HOLD_POSITION
    ...
    def customize_past_chart(self, item):
        ...
        chart['position'] = NOT_AVAILABLE_POSITION
        ...
        # Positions
        # chart.position[0] = OUTSET_POSITION
        # position_count = chart_len - 1
        # for offset in range(1, position_count):
        #     index = chart.index[offset]
        #     if chart.MA5R1_Slope[offset] >= 0:
        #     chart.loc[index, 'position'] = PURCHASE_POSITION
        # chart.MA5R1_Slope[-2]
