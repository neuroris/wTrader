...
'''
Original Algorithm (2021, 10, 18 - 2021.12.07)
...
10. Perfect operation (This version is for preservation)
'''

class FMAlgorithm10(FuturesAlgorithmBase):
    ...
    def start(self, broker, capital, interval, loss_cut, fee, minimum_transaction_amount):
        ...
        # self.timeline_resizer = TimelineResizer(self.trader.timeline_canvas.resize)
        # self.timeline_resizer = TimelineResizer(self.trader)
        self.timer.start()
    ...
    def trade_by_trend(self, amount=0, price=0, order_type='LIMIT'):
        ...
        available_amount = self.episode_amount - open_amount

        self.debug('=========== Trade by Trend =============')
        ...
        self.debug('purchase surplus amount  ', purchase_surplus_amount)
        self.debug('purchase available amount', purchase_available_amount)
        self.debug('purchase order           ', purchase_order_amount)

        # if purchase_order_amount > purchase_available_amount:
        #     purchase_order_amount = purchase_available_amount
        #     self.debug('CHANGED PURCHASE ORDER AMOUNT!!!', purchase_order_amount)
        ...
        self.debug('sale surplus amount  ', sale_surplus_amount)
        self.debug('sale available amount', sale_available_amount)
        self.debug('sale order           ', sale_order_amount)

        # if sale_order_amount > sale_available_amount:
        #     sale_order_amount = sale_available_amount
        #     self.debug('CHANGED SALE ORDER AMOUNT!!!', sale_order_amount)

        if purchase_order_amount > 0:
            self.buy(purchase_order_amount, price, order_type)

        if sale_order_amount > 0:
            self.sell(sale_order_amount, price, order_type)

        self.debug('==================================')

    def trade_by_strangle(self, amount=0, price=0, order_type='LIMIT'):
        if self.long_episode.virtual_open_amount < 0:
            self.long_episode.virtual_open_amount = 0
            self.post_magenta('@ == long episode virtual open amount is negative maybe no further order executed ==')
        if self.short_episode.virtual_open_amount < 0:
        ...
        open_amount = self.long_episode.virtual_open_amount + self.short_episode.virtual_open_amount
        available_amount = self.strangle_episode_amount * 2 - open_amount
    ...
    def strangle_long(self):
        if self.long_episode.virtual_open_amount < 0:
            self.long_episode.virtual_open_amount = 0
            self.post_magenta('@ === virtual open amount is negative no further order executed ===')
        current_purchase_amount = self.long_episode.virtual_open_amount - self.futures.holding_amount
        order_amount = self.strangle_episode_amount - current_purchase_amount
    ...
    def strangle_short(self):
        if self.short_episode.virtual_open_amount < 0:
            self.short_episode.virtual_open_amount = 0
            self.post_magenta('@ === virtual open amount is negative no further order executed ===')
        current_sale_amount = self.short_episode.virtual_open_amount - self.futures.holding_amount
        order_amount = self.strangle_episode_amount - current_sale_amount

        order_capacity = self.futures.holding_amount + self.long_episode.virtual_open_amount
    ...
    def update_execution_info(self, order):
        ...
        # if order.order_position in PURCHASE_EQUIVALENT:
        #     if self.strategy == TREND_STRATEGY:
        #         self.update_trend_long_episode(order)
        #     elif self.strategy == STRANGLE_STRATEGY:
        #         self.update_strangle_long_episode(order)
        # elif order.order_position in SELL_EQUIVALENT:
        #     if self.strategy == TREND_STRATEGY:
        #         self.update_trend_short_episode(order)
        #     elif self.strategy == STRANGLE_STRATEGY:
        #         self.update_strangle_short_episode(order)
    ...
    def update_strangle_long_episode(self, order):
        self.post_episode_info('LONG1', self.long_episode, order)
        ...
        # New episode creation after completion of settlement
        if not self.futures.virtual_holding_amount and order.executed_amount and self.long_episode.executed_amount_sum:
            self.post_red('Completion of settlement')
            self.long_episode = self.long_episode.next_episode('L')
            self.episodes[self.long_episode.episode_number] = self.long_episode
    ...
    def update_strangle_short_episode(self, order):
        self.post_episode_info('SHORT1', self.short_episode, order)
    ...
    def update_trend_long_episode(self, order):
        self.post_episode_info('LONG1', self.long_episode, order)
        ...
        if executed_amount:
            ...
            if self.futures.holding_amount > 0 and \
                ...
                previous_long_episode.order_amount -= discrepancy
                previous_long_episode.executed_amount_sum -= discrepancy
                corrected_purchase_sum = discrepancy * previous_long_episode.purchase_price_avg
    ...
    def update_trend_short_episode(self, order):
        self.post_episode_info('SHORT1', self.short_episode, order)
        ...
        if executed_amount:
            ...
            if self.futures.holding_amount < 0 and \
                ...
                previous_short_episode.order_amount -= discrepancy
                previous_short_episode.executed_amount_sum -= discrepancy
                corrected_purchase_sum = discrepancy * previous_short_episode.purchase_price_avg
    ...
    def display_timeline(self):
        ...
        if xlim > 1150 and self.pause_timeline:
            ...
            # scroll_bar = self.trader.timeline_scroll.horizontalScrollBar()
            # scroll_max = scroll_bar.maximum()
            # scroll_correction = self.timeline.xlim_correction
            # scroll_position = scroll_max - scroll_correction
