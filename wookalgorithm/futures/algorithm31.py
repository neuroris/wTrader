...
'''
Original Algorithm (2021, 12, 16)
** New chart System based on volume!!
...
class FMAlgorithm31(FuturesAlgorithmBase):
    ...
    def update_chart_prices(self, item_code, price, volume):
        ...
        previous_time = chart.index[-1]
        if price > chart.High[-1]:
            chart.loc[previous_time, 'High'] = price
        elif price < chart.Low[-1]:
            chart.loc[previous_time, 'Low'] = price
        last_price = chart.Close[-1]
        chart.loc[previous_time, 'Close'] = price
        chart.loc[previous_time, 'Volume'] += volume

        current_volume = chart.Volume[-1]
        surplus_quota = current_volume // self.quantum_volume
        if surplus_quota:
            chart.loc[previous_time, 'Volume'] = self.quantum_volume
            while surplus_quota:
                chart.loc[current_time] = price
                current_volume -= self.quantum_volume
                if current_volume < self.quantum_volume:
                    chart.loc[current_time, 'Volume'] = current_volume
                else:
                    chart.loc[current_time, 'Volume'] = self.quantum_volume
                microsecond = current_time.microsecond + 1
