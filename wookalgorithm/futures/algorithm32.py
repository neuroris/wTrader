...
10. Trade at consecutive corrected slope(slope + slopeslope)
'''

class FMAlgorithm32(FuturesAlgorithmBase):
    ...
    def market_status(self, item):
        ...
        if item.item_code == self.chart_item.item_code:
            interval = self.kospi200.chart.index[-self.r1_interval:]
    ...
    def trade(self):
        ...
        if self.strategy == TREND_SCALPING_STRATEGY:
            ...
            episode_amount = self.episode_amount
            # purchase_target_amount = episode_amount if self.kospi200.chart.PAR1_Slope[-1] >= 0 else -episode_amount
            # sale_target_amount = episode_amount if self.kospi200.chart.PAR1_Slope[-1] < 0 else -episode_amount
            slope = self.kospi200.chart.PAR1_Slope[-1]
            slopeslope = self.kospi200.chart.PAR1_SlopeSlope[-1]
    ...
    def monitor_trend_deviation(self, current_price):
        ...
        # if chart_len > self.contract_time + 10:
        ...
        order_price = self.get_price_average(self.futures)
        slope = self.kospi200.chart.PAR1_Slope[-1]
        slopeslope = self.kospi200.chart.PAR1_SlopeSlope[-1]
        corrected_slope = slope + slopeslope
        previous_corrected_slope = self.kospi200.chart.PAR1_CorrectedSlope[-2]
        current_corrected_slope = self.kospi200.chart.PAR1_CorrectedSlope[-1]
        if self.futures.holding_amount > 0:
            # if corrected_slope < 0:
            if previous_corrected_slope < 0 and current_corrected_slope < 0:
                order_amount = self.futures.holding_amount - self.short_episode.virtual_open_amount
                if order_amount > 0:
                    self.sell(order_amount, 0, 'MARKET')
        elif self.futures.holding_amount < 0:
    ...
    def update_long_episode(self, order):
        ...
        if executed_amount:
            ...
            # self.long_episode_history[chart_len] = (order.executed_price_avg, self.long_episode.episode_number)
    ...
    def update_short_episode(self, order):
        ...
        if executed_amount:
            ...
            # self.short_episode_history[chart_len] = (order.executed_price_avg, self.short_episode.episode_number)
    ...
    def update_custom_chart(self, item):
        ...
        if chart_len > 2:
            ...
            chart.loc[last, 'PAR1_Slope'] = PAR1_Slope
        ...
        if chart_len > 3:
            ...
            chart.loc[last, 'PAR1_SlopeSlope'] = PAR1_SlopeSlope
            corrected_slope = PAR1_Slope + PAR1_SlopeSlope
            chart.loc[last, 'PAR1_CorrectedSlope'] = corrected_slope
        ...
        chart.loc[last, 'Profit'] = profit
        if self.previous_time == chart.index[-1]:
            ...
            self.previous_time = chart.index[-1]
            self.bet_count += 1

        chart.loc[x3_interval, 'PAR3'] = PAR3
        chart.loc[x3_interval, 'PAR3_Diff'] = chart.PAR3.diff().fillna(0)
        chart.loc[x3_interval, 'PAR3_DiffDiff'] = chart.PAR3_Diff.diff().fillna(0)
        chart.loc[last, 'PAR3_DiffDiffDiff'] = chart.PAR3_DiffDiff[-1] - chart.PAR3_DiffDiff[diff]
        chart.loc[x3_interval, 'MA5R3'] = MA5R3
        chart.loc[x3_interval, 'MA5R3_Diff'] = chart.MA5R3.diff().fillna(0)
        chart.loc[x3_interval, 'MA5R3_DiffDiff'] = chart.MA5R3_Diff.diff().fillna(0)
        chart.loc[last, 'MA5R3_DiffDiffDiff'] = chart.MA5R3_DiffDiff[-1] - chart.MA5R3_DiffDiff[diff]
