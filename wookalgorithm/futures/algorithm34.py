...
class FMAlgorithm34(FuturesAlgorithmBase):
    ...
    def start(self, broker, capital, interval, loss_cut, fee, minimum_transaction_amount):
        ...
        self.kospi200.chart['position'] = numpy.nan
        self.futures.chart['position'] = numpy.nan
    ...
    def market_status(self, item):
        ...
        slope = round(self.kospi200.chart.PAR1_Slope[-1], 3)
        slopeslope = round(self.kospi200.chart.PAR1_SlopeSlope[-1], 3)

        chart = self.kospi200.chart
        # interval = chart.index[-10:]
        # self.debug(self.kospi200.chart.loc[interval, 'position'])

        if item.item_code == self.chart_item.item_code:
            interval = self.kospi200.chart.index[(self.r1_interval * -2):]
            # self.debug(self.kospi200.chart.loc[interval, 'PAR1_Slope'])
            # self.debug(self.kospi200.chart.loc[interval, 'PAR1_SlopeSlope'])
            # self.debug(self.kospi200.chart.loc[interval, 'PAR1_CorrectedSlope'])
            # # self.debug('sum', round(slope + slopeslope, 3))
            # self.debug(self.kospi200.chart.loc[interval, 'Profit'])
            # # self.debug('profit', self.kospi200.chart.Profit[-1])
            # # self.debug('previous accumulated profit', self.previous_accumulated_profit)
            # self.debug('accumulated profit({})'.format(self.bet_count), self.accumulated_profit)

            self.debug(chart.loc[interval, ['PAR1_Slope', 'PAR1_SlopeSlope', 'PAR1_CorrectedSlope', 'position']])
            self.debug('accumulated profit({})'.format(self.bet_count), self.accumulated_profit)
        ...
        # self.debug('Slope', round(self.chart.PAR1_Slope[-1], 5), 'SlopeSlope', round(self.chart.PAR1_SlopeSlope[-1], 5))
    ...
    def monitor_trend_deviation(self, current_price):
        ...
        current_slope = chart.PAR1_CorrectedSlope[-2]
        current_time = chart.index[-2]
        previous_position = chart.position[-self.r1_interval - 2]
        if current_slope >= 0:
        ...
        # chart = self.kospi200.chart
        # current_slope = chart.PAR1_CorrectedSlope[-1]
        # current_time = chart.index[-1]
        # previous_position = chart.position[-self.r1_interval - 1]
        # if current_slope >= 0:
        #     chart.loc[current_time, 'position'] = PURCHASE
        #     if previous_position == SELL:
        #         self.buy(self.bet_amount * 2, 0, 'MARKET')
    ...
    def customize_past_chart(self, item):
        ...
        for offset in range(slope_count):
            ...
            x, R1 = self.get_linear_regression(chart, chart.PA, interval, offset)
            slope = numpy.polyfit(x, R1, 1)[0]
            index = chart.index[-1 - offset]
            chart.loc[index, 'PAR1_Slope'] = slope
        ...
        # PAR1 Corrected Slope
        for offset in range(slopeslope_count):
            index = -1 - offset
            slope = chart.PAR1_Slope[index]
            slopeslope = chart.PAR1_SlopeSlope[index]
            corrected_slope = slope + slopeslope
            index = chart.index[index]
